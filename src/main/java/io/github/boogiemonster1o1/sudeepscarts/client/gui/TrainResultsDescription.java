...
public class TrainResultsDescription extends FullscreenGuiDescription {
	...
	public TrainResultsDescription(TrainsStatus status) {
		...
		root.add(trainList, 20, 40, 400, 200);

		root.validate(this);
	}

	public static class WTrainElement extends WPlainPanel {
		public WTrainElement() {
			this.setBackgroundPainter(BackgroundPainter.createColorful(0x55000000));
		}

		public void configure(TrainBetweenStations train) {
			this.add(new WLabel(Text.literal(train.trainName() + " : " + train.fromStationName() + " - " + train.toStationName())), 0, 0);
		}
	}
